...
def Make_hyp (hashtag, my_dict):
	...
	while (flag):
		...
		while it < len(word_hyp):
			#print(word_hyp)
		#for it in range (0,len(word_hyp)):
						...
						if not (substring[0] in alphabet):
							...
							if len(substring) >= 1:
								for i in range (0, len(substring)):
										...
										if not (first_word_hyp_existing(subsubstring, my_dict) == []):
											if (first_variant):
												first_variant = False
												mem = word_hyp[it][:]
											...
											flag = True
										unknown_finish = i+1
								if len(substring) == unknown_finish and first_variant:
...
#print(Make_hyp('1000000000извинений', Dict_creator.Dict_form('/home/corra/Documents/VKR/zaliznjak.txt')))
